...
BOOST_AUTO_TEST_CASE(GetFeeTest)
{
    ...
    // due to only supporting roshis per kB, so CFeeRate(CAmount(1), 1001) was equal to CFeeRate(0)
...
BOOST_AUTO_TEST_CASE(BinaryOperatorTest)
{
    ...
    BOOST_CHECK(b >= a);
    BOOST_CHECK(b >= b);
    // a should be 0.00000002 RNG/kvB now
    a += a;
    BOOST_CHECK(a == b);
}
...
BOOST_AUTO_TEST_CASE(ToStringTest)
{
    CFeeRate feeRate;
    feeRate = CFeeRate(1);
    BOOST_CHECK_EQUAL(feeRate.ToString(), "0.00000001 RNG/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::BTC_KVB), "0.00000001 RNG/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::SAT_VB), "0.001 roshi/vB");
}
